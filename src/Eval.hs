...
matchCond (IntLit i:e1s) (IntLit j:e2s) guard varMap env = if i == j then matchCond e1s e2s guard varMap env else pure False
matchCond (DoubleLit i:e1s) (DoubleLit j:e2s) guard varMap env = if i == j then matchCond e1s e2s guard varMap env else pure False
matchCond (StrLit i:e1s) (StrLit j:e2s) guard varMap env = if i == j then matchCond e1s e2s guard varMap env else pure False
matchCond ((ListLit l1 _):e1s) ((ListLit [Var h _, Var t _] _):e2s) guard varMap env = matchCond e1s e2s guard varMap env
matchCond ((ListLit l1 _):e1s) ((ListLit l2 _):e2s) guard varMap env = if l1 == l2 then matchCond e1s e2s guard varMap env else pure False
...
matchCond e1 e2 _ varMap env = trace ("matchCond: " ++ show (e1,e2)) $ pure False
