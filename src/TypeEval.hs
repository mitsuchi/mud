...
typeEval (Apply (Var name code) args) env = do
  ...
  types <- case fun' of
    ...
    _ -> throwError $
         ...
         ++ name ++ " : " ++ intercalate " -> " (map rArrow args')
         ++ " -> ?' not found"
  ...
  case unify (rInit ts) xs mempty of
    Nothing -> throwError $
               ...
               ++ "ts:" ++ show ts
               ++ ",xs:" ++ show xs
               ++ ":fugafuga type mismatch. function '"
               ++ name ++ " : "
               ++ intercalate " -> " (rArrow <$> args')
               ++ " -> ?' not found."
  ...
  if cond' == Elem "Bool"
    then if then' == else'
      then pure then'
           ...
           show (lineOfCode code)
           ++ ": type mismatch. then-part has a type '"
           ++ rArrow then'
           ++ "', else-part has '"
           ++ rArrow else'
         ...
         show (lineOfCode code)
         ++ ": type mismatch. condition-part has a type '"
         ++ rArrow cond'
         ++ "'. must be 'Bool'."
typeEval (FunDef nameExpr@(Var name code) types params body) env = do
  ...
  case unify types (rAppend (rInit types) (Elems [body'])) mempty of
    ...
    Nothing -> throwError $
               show (lineOfCode code)
               ++ ": type mismatch. function supposed to pure '"
               ++ rArrow (rLast types)
               ++ "', but actually pures '"
               ++ rArrow body' ++ "'"
...
typeInst (Elem a) env | isUpper (head a) = Elem a
                      | isLower (head a) = fromMaybe (Elem a) $ env !? a
typeInst (Elems es) env = Elems ((`typeInst` env) <$> es)
