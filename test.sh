...
try "40" "fun twice : (a->a)->a->a = f x -> f (f x); twice (x->x+x:a->a) 10"
try "bbbb" "fun twice : (a->a)->a->a = f x -> f (f x); twice (x->x+x:a->a) 'b'"
...
try -3 "-3"
try -48 "-4*12"
try 2.2 "fun add : Double -> Double -> Double = { x y -> x + y }; add 1.1 1.1"
...
try 4 "type Complex = { r:Int, i:Int }; fun add : Complex -> Complex -> Complex = x y -> Complex (x.r+y.r) (x.i+y.i); a = Complex 1 2; b = Complex 3 4; (add a b).r"
try 4 "type Complex = { r:Int, i:Int }; fun + : Complex -> Complex -> Complex = x y -> Complex (x.r+y.r) (x.i+y.i); a = Complex 1 2; b = Complex 3 4; (a + b).r"
try 8 "type Complex = { r:Int, i:Int }; fun * : Complex -> Complex -> Complex = x y -> Complex (x.r*y.r) (x.i*y.i); a = Complex 1 2; b = Complex 3 4; (a * b).i"
...
try 2 "type Complex = { r:Int, i:Int }; fun - : Complex -> Complex -> Complex = x y -> Complex (x.r-y.r) (x.i-y.i); a = Complex 1 2; b = Complex 3 4; (b - a).r"
try 3 "fun * : (b->c) -> (a->b) -> (a->c) = f g -> (x -> x.g.f : a -> c); fun inc : Int -> Int = x -> x + 1; fun double : Int -> Int = x -> x * 2; (inc * double) 1"
